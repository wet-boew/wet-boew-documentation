...
The following are an updated and improved version of the Design Decision #3 about WET versioning API. It provides more details about what is versioned for the wet-boew product, GCWeb product and GCWeb Jekyll product.

The [Semantic Versioning 2.0](https://semver.org/) is followed and applied.

## Versioned

Here the items of our product that we are monitoring and illustrating changes via our version number.
...
* Fully in scope:
	...
	* Widely use template
* Identified at must a minor change with a breaking change when
	* Use moderated and accompanied with an official communication out explicitly identified in the release notes
* Identified at must a patch change with a breaking change when
	* Use limited and all its active instances are explicitly listed in its corresponding template documentation.
...
* Limited use: There is a few instance which are all explicitly listed in its corresponding template documentation.

Audience: Web author creating pages with the wet-boew product in a static server side environment.
