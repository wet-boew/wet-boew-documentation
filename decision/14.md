...
## This is a work in progress completed at ~80%

Todo:

* Revise this document to ensure consistency.
* Describe the target audience (author, implementer, developer, users) and list dimension changes that impact their role.
* Create a change impact evaluation checklist.
* Design and automate the component documentation based on the observable dimension.
...
Identified as minor
* Provisional feature or provisional variant that are added or deprecated with a downgrade (functionality made available) in an external snippet
...
A variant is a distinctive alternate configuration which was identified (by iteration number) of a measured component. For a given component it might define several active supported "iteration" where each active one can be recognized as a version of the component.

### Variant identification (iteration)

The variant iteration identification can be defined by one of more of the following properties where it remain unique in the component scope:

* Iterative number: An incremental number.
* Observed on: A reference date of when the informational variant details was captured and documented.
* Version number: A tag usually starting with the prefix "v" followed by a semantic version number, like: "v1.2.3". Or a date like 2022-11-16.
...
* Provisional: Experimental feature in a good standing to be stabilized
  - Version API impact for the component:
	...
	* Minor: The provisional variant is added or officially deprecated with a downgraded (functionality made available) in an external snippet.
	* Patch: The provisional variant is updated or removed after it's official deprecation.
...
* Deprecated
  - Version API impact for the component:
    * Major: A stable variant is deprecated or removed.
	* Minor: A provisional variant is deprecated with a downgraded (functionality made available) in an external snippet
	* Patch: A provisional variant is removed. Or an informational variant is deprecated or removed
  - Version API impact for the product:
    * Major: When the backward compatible code of a variant is fully removed
	* Minor: When a new variant is added
	* Patch: The variant pattern is updated or review.
