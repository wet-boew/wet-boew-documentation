			...
			fetch(elm.dataset.wbData).then(
				function(response) {
					...
					response.json().then(function(data) {
						...
						var loadedComponentName = data.class;		//class name
						var dtEventObj = data.data;					//data of component (text, images, etc)
						var template = templates[data.template];	//template of component (see templates.json)
						var dataObject = wb5React( dtEventObj );	//make data reactive (this is not useful ATM)
						...
						//Initialise the new plugin - This does not work properly, for reasons unknown
						$("[id='"+dataObject.data["id"]+"']").trigger( "wb-init." + loadedComponentName );
						//wb.add( "." + loadedComponentName );
