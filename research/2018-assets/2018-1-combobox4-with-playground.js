...
$document.on( "click", "#render", function(){
	...
	// Get the JS object
	var jsObject = new Function( 'return ' + doc.getElementById( "jsobj" ).value );
	...
	var templateString = doc.getElementById( "tmpl" ).value, // Template as text
		clone,
	...
	doc.getElementById( "results" ).appendChild( clone );
...
$doc.on( "combobox.createctrl.wb2-fieldflow", ".wb2-fieldflow", function( event, data ) {
	...
	window[ "wb-combobox" ].compareMe = function( a, b ) {
		return a.toLowerCase().indexOf( b.toLowerCase() ) !== -1;
	};

	// 
	window[ "wb-combobox" ].similarText = function( s1, s2, passRatio ) {

		// Function to compare the distance between two word.
		function compareRate( s1, s2 ) {
			...
			for ( var i = 0; i <= longerLength; i++ ) {
				...
				if ( i > 0 ) {
					arr[shorterLength] = lastValue;
				}
			}
			distance = arr[ shorterLength ];

			return ( ( longerLength - distance ) / parseFloat( longerLength ) ) * 100;
		}

		// Calculating the distance for each term and check if it pass the passRatio
		var lstS1 = s1.split( " " );
		var lstS2 = s2.split( " " );
		...
		for ( i = 0; i !== lstS1.length; i++ ) {
			var j;

			for( j = 0; j !== lstS2.length; j++ ) {
				var result = compareRate( lstS1[ i ], lstS2[ j ] );
