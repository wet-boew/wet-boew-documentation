...
var componentName = "wb2-fieldflow",
...
// console.log ("CHANGE called ---- in Field Flow" );
...
// console.log( "Fieldflow ACTION: " + actions.length);
// console.log( actions );
...
// If there is If None setting
			if ( !actions && settings.defaultIfNone ) {
			 	// run the default If None action
			 	...
			 	for ( m = 0, m_len = actions.length; m !== m_len; m += 1 ) {
			 		...
			 		m_cache.provEvt = m_cache.$selElm.get( 0 ); // Get the first child
			 		m_cache.form = elm;
...
$document.on( fieldflowActionsEvents, selector, function( event, data ) {
	...
	case submitEvent:
		...
		case "redir":
			...
			// console.log( "SUBMIT EVENT - REDIR action to get executed.........." )
